...
bool GPTJ::loadModel(const std::string &modelPath, int32_t n_threads, int32_t seed)
{
    if (seed < 0)
        seed = time(NULL);
...
void GPTJ::prompt(const std::string &prompt, std::function<bool(const std::string &)> response,
                  PromptContext &ctx, int32_t n_predict, int32_t top_k, float top_p, float temp, int32_t n_batch, bool verbose)
{
